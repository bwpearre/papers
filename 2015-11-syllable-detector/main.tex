...
We obtain both of these numbers by performing a maximum-likelihood fit of a Gaussian distribution to the timing data obtained from the oscilloscope.  We refer to the distribution's mean as latency and standard deviation as jitter.
...
computation, and FFT time shift aliasing, but excluding song alignment jitter and detection variability.
...
\begin{figure}
  ...
  \noprint{Each subsequent plot shows one network output unit's responses to all 2818 presentations of the song. The horizontal axis is time relative to the beginning of the aligned song segment and corresponds to the top figure, and the
...
\marginpar{Needed: song+detection rasters for at least a couple of the syllables}
...
Our 1.5-ms--frame default is a compromise: faster frames increase computational requirements both at runtime and during training, but also increase the precision of timing.  We evaluate our timing at our default, and then show how performance is affected across three variables:
\begin{itemize}
\item FFT frame interval
\item Syllable
\item Detector
\end{itemize}
...
\begin{figure}
  ...
  \caption{Timing varies as the FFT frame interval changes.  Here we show results for the ``Ideal'' detector and the LabView and Swift+serial implementations, for the constructed $\delta$-song and for the real bird song at 300 ms.}
  \label{fig:TimingVsFrame}
\end{figure}

Detector latency and jitter depend on the FFT frame rate.  \fig{fig:TimingVsFrame} shows how these numbers vary over a useful range of frame rates on our Ideal detector, the Swift detector with serial output, and for the LabView detector.

\subsubsection{Real song}

\begin{figure}
  \includegraphics[width=\textwidth]{TimingVsSyllable}
  \caption{Syllable choice impacts detector performance, but despite the variety of syllables chosen here, performance was fairly stable across syllables.}
  \label{fig:TimingVsSyllable}
\end{figure}

\marginpar{FFT interval? Show delta, and show average across our real syllables}

\subsubsection{Detector Implementations}

\begin{figure}
  \includegraphics[width=\textwidth]{TimingVsDetector}
  \caption{The different detectors for the constructed $\delta$-song and for the real bird song at 300 ms.}
  \label{fig:TimingVsDetector}
\end{figure}

Again standardising on our 1.5-ms FFT frame interval, \fig{fig:TimingVsDetector} shows latency and jitter across our detector implementations for the $\delta$-song and for our bird syllable at 300 ms.

\begin{figure}
  \includegraphics[width=7cm]{timing}
  \caption{Timing curve for a syllable running a test file generated by training.}
  \label{fig:timing}
\end{figure}


\fig{fig:timing} gives a more detailed view of what the timing curves look like for the five implementations of our detector trained on our test bird, for the syllable at 300 ms, using an FFT frame rate of 1.5 ms.  The maximum-likelihood estimates of the mean $\mu$ and standard deviation $\sigma$ of the detector latencies on this training set are as follows:\marginpar{Still useful?  Update, anyway!}
...
\begin{tabular}{l|c|c}
  ...
  Ideal & 2.3 & 2.0 \\
  LabView & 3.2 & 1.4\\
  Swift+serial & 4.7 & 1.9 \\
  Swift+audio & 8.9 & 1.9 \\
  Matlab+serial & 8.7 & 2.5 \\
  Matlab+audio & 16 & 2.2
\end{tabular}

\vspace{8pt}

The Swift detector with serial out and the LabView implementation are approximately equivalent in performance.  We greatly prefer the Swift implementation due to its lower hardware and software requirements and the difficulty of modifying and debugging LabView programmes---indeed, the LabView implementation still contains a bug that slightly decreases its accuracy compared to the Swift and Matlab detectors.  With serial output, Matlab's performance is good, although its buffer handling is highly sensitive to system load.

Due to the randomness of neural network initialisation and training song presentation, multiple training runs yield different networks.  We found that occasionally a network would perform unusually poorly, but that it was easy to spot these networks by their performance on the test audio file, and re-train when necessary.
