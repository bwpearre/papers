...
spectrogram.  The set of frequencies $F$ and frames
$T$ of this recognition region may be chosen in order to contain
unique features of the target syllable and surrounding areas.  We have found that a 30-ms time window (which will yield $|T|=\lfloor 30\mbox{ ms}/t_f\rceil$ frames) of frequencies spanning 1--8 kHz works well for most syllables.
...
at each time $t$ the rectangular $|F|\times |T|$ recognition region in the spectrogram is
simply reshaped into a vector $x_t$ of length $|F||T|$. The frequency range is constant, and every possible time interval of length $|T|$ at time $t$ is converted into a training input vector $x_t$.  These are placed into a training matrix, $X$, in which each column is one $x_t$, and each row is one ``pixel'' of the spectrogram image.
...
% this sentence is easy to miss in the paper, it might make sense to move it before the table
