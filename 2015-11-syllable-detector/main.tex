...
\begin{abstract}
  ...
  experiments require that specific moments during song be identified in real time
...
\item Accurate: false positive rates under 0.02\%, and false negative rates well under 1\%, for a variety of syllables.  Balancing these rates depends on a single relative-cost parameter.
...
The spectrogram is computed at regular intervals---usually every 1-5
milliseconds, which we refer to as the frame time
$t_f$.  The examples in this paper use $t_f=1.5$ ms.
...
alignment may not be perfect, due to sample aliasing, and because the song spectrogram appears not to vary faster than the frame rate we chose, a strict binary target may ask the network to learn that practically identical samples should have opposite targets. Thus it is preferable to spread the output
...
nodes and autoencoders. Perhaps surprisingly, even the former yields excellent results
on many syllables, but a 2-layer perceptron with a very small hidden
layer---with a unit count 3 times the number of target
...
We divided the data into training and test sets.  For the results reported in the following section, the training set consisted of 1000 instances of the song of one bird and 2000 instances of non-song data.  Our test set consisted of an additional 1818 iterations of the bird's song and 3636 more instances of non-song data.

The Matlab neural network toolbox further divides our ``training'' set into internal training, test, and final validation sets.  We did not stray from the toolbox's default values, and do not discuss Matlab's internal subdivision of our training set.
...
fig{fig:sixsyllables} shows a typical song.  The image was created by averaging the aligned spectra of 2818 songs.  Three target syllables are shown, at 150, 315, and 405 milliseconds after the beginning of the aligned samples, as indicated by the red lines.  The rectangles show the recognition window (30 ms, 1-8 kHz) for each instant in the song.

The first trigger, at 150 ms, is far from a pure tone, but is closer to white noise over the region.  It would be difficult for a harmonic-stack filterbank to detect.  The second trigger, at 315 ms, is a more typical harmonic stack amenable to a variety of recognition techniques, although consistently detecting a point midway through the syllable demands a detector that can use multiple timesteps.  The third syllable, at 405 ms, shows a steady pitch followed by a downward slide along with a change in timbre, and was chosen in order to test the detector's ability to discriminate between it and the similar region at 250 ms.


\subsection{Accuracy}

We allowed our training software to allocate our default of 3 hidden units per syllable, for a total of 9.  n order to eliminate the large number of variables involved in microphone, cage, playback and re-digitising, we first evaluated the neural network's accuracy directly on the digitised recording.  When training data and runtime data are gathered on the same hardware setup, this is the signal that the detector will see---only the non-song data may be different.  Accuracy \marginpar{Should be! Verify! And/or explain that there is a bug in the LabView implementation, if there in fact is. ALSO need to present a picture of the test syllable or etc\dots} is identical across the three detectors.  Finally, the relative cost of false negatives, $C_n$ (described in \sref{sec:optimalthresholds}), is set to the default value of 1.  The confusion matrices are:n

\vspace{8pt}\par\noindent
\begin{tabular}{r|cc}
  {\bf At 150 ms} & \multicolumn{2}{c}{True} \\ 
  & pos & neg \\ 
  \hline  Detected pos & 99.66611\% & 0.00153\%\\ 
  neg & 0.33389\% & 99.99847\%\\ 
\end{tabular}
\vspace{8pt}\par\noindent
\begin{tabular}{r|cc}
  {\bf At 315 ms} & \multicolumn{2}{c}{True} \\ 
  & pos & neg \\ 
  \hline  Detected pos & 99.83306\% & 0.00024\%\\ 
  neg & 0.16694\% & 99.99976\%\\ 
\end{tabular}
\vspace{8pt}\par\noindent
\begin{tabular}{r|cc}
  {\bf At 405 ms} & \multicolumn{2}{c}{True} \\ 
  & pos & neg \\ 
  \hline  Detected pos & 99.66611\% & 0.00243\%\\ 
...
\begin{figure}
  \includegraphics[width=8cm]{song}
    \caption{Each plot shows detection events for
      the corresponding syllable. The horizontal axis is time relative to the beginning of the aligned song segment, and the
      vertical axis is the index of the 2818 single song
      ...
      cyan regions and unseen test songs to the right of red
...
\item Works on a wide range of target syllables using the default values evaluated here, generally eliminating the need for hand-tuning.
...
Duration detection is similarly easy.  The beginning and end of a syllable may be detected by looking at the ratio of total energy in the singing frequency band (approximately 1-8 kHz) to the total energy, over some small time window.  Any syllable thus identified that also contains a trigger event may be monitored for duration.  Alternatively, the network can be trained to recognise both the beginning and the end of the syllable of interest.
