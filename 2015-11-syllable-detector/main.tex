...
\begin{abstract}
  ...
  moments during song can be isolated and detected with $>99$\%
  accuracy, easier syllables can be detected $~99.7\%$ of the time,
...
Since the network may produce above-threshold responses to nearby frames, after the first response, subsequent responses are suppressed for 100 ms.\marginpar{Nathan, what de-bounce time do you use?} However, for measuring accuracy, we used the un-de-bounced network response.
...
detection \marginpar{FIXME New syllables coming, with more thorough analysys.}  performance for each iteration of the song is shown
...
In order to eliminate the large number of variables involved in microphone, cage, playback and re-digitising, we first evaluated the neural network's accuracy directly on the digitised recording.  When training data and runtime data are gathered on the same hardware setup, this is the signal that the detector will see---only the non-song data may be different.
...
The song used in our case study is shown in \fig{fig:songspectrogram}.  Three target detection points are shown, at 150, 315, and 402 milliseconds on the axis (whose significance is only that that was the portion identified as song during alignment).  The first syllable is difficult for harmonic-stack--style recognition since it does not contain any easily identifiable pitch.  The second, at 315ms, should be relatively easy.  The third, at 402ms, is difficult for instantaneous detectors as it contains a downward pitch shift, and furthermore it is similar to the song at 235ms.

As discussed in \S{sec:optimalthresholds}, the false-positive and false-negative rates may be traded off against each other through a relative-cost parameter.  We present results with two values of $C_n$, the cost of false negatives relative to false positives.
...
\begin{figure}
  \includegraphics[width=\textwidth]{song}
