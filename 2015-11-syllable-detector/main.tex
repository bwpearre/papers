...
recorded via fiber photometry to identify a neural representation of 
...
to pilot our novel, neural network-based syllable detector.
...
After injecting the virus, we implanted a fiber-optic cannula above 
HVC ($400 \mu m$ diameter, Doric Lenses). The location of the injection 
was verified histologically after each experiment.

\subsubsection{Photometry}

Fiber photometry \cite{Adelsberger:2005dy,Schulz:2012fv,Cui:2013dq,Adelsberger:2014jd,Gunaydin:2014dh} 
was performed in N=3 birds at least 2 weeks post-injection using a 
completely fiber-coupled light path. A 470 nm fiber-coupled LED (Mightex) 
...
was connected via a multimode fiber optic patch cord ($400 \mu m$, .48 NA, 
...
The neural network-based syllable detector provided improved 
performance over hand-tuned filter-banks
experiment, by eliminating the need for manual tuning, 
increasing the types of syllables that could be targeted and 
decreasing the error rate. \fig{fig:fp-audio} shows 1,123 
song performances from a single bird. At trial 858, the altered
auditory feedback (AAF) was introduced, triggered by the first syllable
in the song.
...
shelf that respond to AAF and potentially encode an error signal based
on auditory feedback during song performance.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=6cm]{fp-audio}
  \end{center}
...
\begin{figure}
  \begin{center}
    \includegraphics[width=6cm]{fp-results}
  \end{center}
  \caption{Fiber photometry during AAF reveals an AAF-sensitive response
