...
pulse, that can be used to trigger experimental feedback.
...
Given those definitions, we can compute the false-positive and false-negative rates. We present the rates for those metrics over a few songs, and, for simplicity, we present a summary of detector accuracy using the area under the ROC curve.
...
there in the latency? This we obtain by eyeballing the $\mu\pm 34\%$ crossings ($1\sigma$) of the
...
single low-to-high step function).  Each grid square is 2.5ms long, so
average detection latency appears to be around 5ms, while jitter
appears to have $\sigma\simeq 1.5$ms or so (eyeballed).
...
\begin{figure}
  ...
  \caption{Alternate view of timing, from the LabView implementation.}
